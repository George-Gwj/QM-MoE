    ...
    if args.num_steps == -1:
        with torch.inference_mode():
            while True:
                ...
                # # 确保使用正确的维度
                # dof_pos_tensor = env.gym.acquire_dof_state_tensor(env.sim)
                # dof_pos = gymtorch.wrap_tensor(dof_pos_tensor).view(env.num_envs, -1, 2)[..., 0]

                # # 使用实际的DOF数量，而不是假设与num_actions相等
                # actual_num_dofs = dof_pos.shape[1]
                # dof_names = env.gym.get_actor_dof_names(env.envs[0], env.actor_handles[0])

                # print("DOF名称与角度（第0个环境）:")
                # for name, pos in zip(dof_names, dof_pos[0].cpu().numpy()):
                #     print(f"{name}: {pos:.4f}")
                # update_cam_pos()
                env.render()
