...
def plot_velocity_curves(velocity_logs, output_dir):
    ...
    # 角速度曲线
    plt.subplot(2, 1, 2)
    plt.plot(df["time"], df["desired_ang_vel_z"], 'r-', label='Desired Z Angular Velocity', linewidth=2)
    plt.plot(df["time"], df["actual_ang_vel_z"], 'b-', label='Actual Z Angular Velocity', linewidth=1.5)
    plt.xlabel('Time (s)')
    plt.ylabel('Angular Velocity (rad/s)')
    plt.title('Angular Velocity around Z Axis')
    plt.legend()
    plt.grid(True)
...
def play():
    ...
    # config["env"]["tasks"]["locomotion"]["pitch_range"] = [0.0, 0.0]
    # config["env"]["tasks"]["locomotion"]["roll_range"] = [0.0, 0.0]
    # config["env"]["tasks"]["locomotion"]["z_height_range"] = [0.3, 0.3]
    ...
    # 获取期望速度值
    desired_lin_vel_x = config["env"]["tasks"]["locomotion"]["lin_vel_range"][0][0]
    desired_ang_vel_z = config["env"]["tasks"]["locomotion"]["ang_vel_range"][2][0]
    ...
    if args.num_steps == -1:
        with torch.inference_mode():
            while True:
                ...
                #     actual_lin_vel_x = env.state.root_lin_vel[actor_idx, 0].item()
                #     actual_ang_vel_z = env.state.root_ang_vel[actor_idx, 2].item()
                    
                #     velocity_logs["time"].append(current_time)
                #     velocity_logs["desired_lin_vel_x"].append(desired_lin_vel_x)
                #     velocity_logs["actual_lin_vel_x"].append(actual_lin_vel_x)
                #     velocity_logs["desired_ang_vel_z"].append(desired_ang_vel_z)
                #     velocity_logs["actual_ang_vel_z"].append(actual_ang_vel_z)
