...
class IsaacGymEnv(VecEnv):
    ...
    def step(
        ...
        if self.cfg.domain_rand.push_robots and (
            ...
            # 只影响机器人的状态（偶数索引：0, 2, 4, ...）
            robot_actor_ids = torch.arange(0, self.num_envs * 2, 2, device=self.device)
    ...
    def _reset_root_states(self, env_ids):
        ...
        # 重置box状态（奇数索引：1, 3, 5, ...）
        box_actor_ids = env_ids * 2 + 1
        self._reset_box_states_in_root_state(env_ids, box_actor_ids)
        
        # 设置所有actor的状态（包括机器人和box）
        num_actors_per_env = 2
        # 创建包含机器人和box的actor索引
        all_actor_ids = torch.cat([
            robot_actor_ids,  # 机器人actor索引（偶数）
            box_actor_ids     # box actor索引（奇数）
    ...
    def _create_envs(self):
        ...
        box_asset = self.gym.create_box(
            self.sim, 0.5, 0.5, 0.05, box_asset_options
        )
        ...
        for i in range(self.num_envs):
            ...
            # 尝试不同的偏移方向来找到机器人的"前方"
            # 使用更大的偏移来确保Box在机器人前方
            box_offset = gymapi.Vec3(2.0, 0.0, 0.3)  # 前方3米，上方0.5米
            
            # 计算box的绝对位置：机器人位置 + box相对位置
            box_absolute_pose = gymapi.Transform()
            box_absolute_pose.p = start_pose.p + box_offset
            box_absolute_pose.r = start_pose.r  # 使用机器人的旋转
    ...
    def _reset_dofs(self, env_ids):
        ...
        env_ids_int32 = (env_ids * 2).to(dtype=torch.int32)
    ...
    def _reset_box_states_in_root_state(self, env_ids, box_actor_ids):
        """在root_state中重置box物体的位置和状态"""
        for i, env_id in enumerate(env_ids):
            if env_id < len(self.box_handles):
                # 获取机器人的位置（世界坐标）
                robot_actor_id = env_id * 2
                robot_pos = self.state.root_state[robot_actor_id, 0:3]
                
                # Box相对于机器人的位置：在机器人前方2.0米，上方0.0米
                # 直接在世界坐标系中计算，与创建时的逻辑一致
                box_offset = torch.tensor([2.0, 0.0, 0.3], device=self.device)
