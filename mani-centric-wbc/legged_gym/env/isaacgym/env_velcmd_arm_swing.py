...
class IsaacGymEnvVelCmdArmSwing(IsaacGymEnv):
    def __init__(
        ...
        self.curriculum_min_stable_steps = getattr(self.cfg.curriculum, 'min_stable_steps', 5000)  # 最少稳定步数
    ...
    def _evaluate_performance(self):
        ...
        # 计算机械臂目标跟踪误差
        arm_target_error = 0.0
        if self.arm_dof_indices and self.arm_random_pose_enabled:
            arm_errors = []
            for i, dof_idx in enumerate(self.arm_dof_indices):
                joint_error = torch.abs(self.state.dof_pos[:, dof_idx] - self.arm_target_pos[:, i])
                arm_errors.append(torch.mean(joint_error).item())
            arm_target_error = sum(arm_errors) / len(arm_errors) if arm_errors else 0.0
        ...
        logging.info(f"性能评估 - 基座速度误差: {base_cmd_error:.4f}, 机械臂目标误差: {arm_target_error:.4f}, 姿态误差: {orientation_error:.4f}, 当前速度指令: {self.base_lin_vel_cmd},当前阶段: {self.curriculum_stage}")
    ...
    def _check_stage_requirements(self, base_error, arm_error, orientation_error):
        ...
        if self.curriculum_stage == 1:
            ...
            # 阶段2：基座速度为0，机械臂目标跟踪和姿态稳定
            return (base_error < self.curriculum_thresholds['base_cmd_tracking_error'] and
                    arm_error < self.curriculum_thresholds['arm_target_tracking_error'] and
                    orientation_error < self.curriculum_thresholds['orientation_error'])
        
        elif self.curriculum_stage == 3:
            # 阶段3：基座速度30%范围，需要基座速度跟踪
            return (base_error < self.curriculum_thresholds['base_cmd_tracking_error'] and
                    arm_error < self.curriculum_thresholds['arm_target_tracking_error'] and
                    orientation_error < self.curriculum_thresholds['orientation_error'])
        
        elif self.curriculum_stage == 4:
            # 阶段4：基座速度60%范围，需要基座速度跟踪
            return (base_error < self.curriculum_thresholds['base_cmd_tracking_error'] and
                    arm_error < self.curriculum_thresholds['arm_target_tracking_error'] and
                    orientation_error < self.curriculum_thresholds['orientation_error'])
    ...
    def _advance_curriculum_stage(self):
        ...
        if self.curriculum_stage == 1:
            ...
            self.arm_pose_range_scale = 0.5
            self.base_vel_cmd_scale = 0.0
            stage_description = "基座速度为0，机械臂目标关节角度50%范围（开始机械臂训练）"
        
        elif self.curriculum_stage == 3:
            self.arm_pose_range_scale = 0.5
            self.base_vel_cmd_scale = 0.3
            stage_description = "基座速度30%范围，机械臂目标关节角度50%范围"
        
        elif self.curriculum_stage == 4:
            self.arm_pose_range_scale = 0.5
            self.base_vel_cmd_scale = 0.6
            stage_description = "基座速度60%范围，机械臂目标关节角度50%范围"
        
        elif self.curriculum_stage == 5:
            self.arm_pose_range_scale = 0.5
            self.base_vel_cmd_scale = 1.0
            stage_description = "基座速度100%范围，机械臂目标关节角度50%范围"
    ...
    def _reward_arm_target_tracking(self, state: EnvState, control: Control):
        ...
        # 第一个课程阶段：机械臂奖励系数为0（只训练四足站立）
        if self.arm_reward_scale_by_stage and self.curriculum_stage == 1:
            return torch.zeros(self.num_envs, device=self.device, dtype=torch.float)
        
        arm_target_error = torch.zeros(self.num_envs, device=self.device, dtype=torch.float)
        for i, dof_idx in enumerate(self.arm_dof_indices):
            joint_error = state.dof_pos[:, dof_idx] - self.arm_target_pos[:, i]
            arm_target_error += torch.square(joint_error)
        
        return torch.exp(-arm_target_error / self.arm_target_tracking_sigma**2)
