...
def play():
    ...
    # # 1. 深拷贝得到policy_2（确保参数和结构完全独立）
    # policy_2 = copy.deepcopy(policy)

    # # 2. 确保两个策略在相同设备上（如GPU）
    # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    # # policy = policy.to(device)
    # # policy_2 = policy_2.to(device)

    # # 3. 环境交互循环（示例）
    # # env = YourEnvironment()  # 你的环境
    # # obs = env.reset()  # 初始观测
    # weights = torch.tensor([0.7, 0.3], device=device)  # 融合权重（可动态调整）
    if args.num_steps == -1:
        with torch.inference_mode():
            while True:
                actions = policy(obs)#policy
                # actions2 = policy_2(obs) 
