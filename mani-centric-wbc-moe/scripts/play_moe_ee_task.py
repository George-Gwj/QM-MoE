...
def check_checkpoint_type(ckpt_path):
    """检查检查点文件类型，判断是否为MoE模型"""
    try:
        ...
        # 检查是否包含MoE相关层
        moe_keys = ['mlp_composer_net', 'policy1', 'policy2']
        has_moe_layers = any(key in state_dict for key in moe_keys)
        
        return has_moe_layers
    except Exception as e:
        print(f"检查检查点类型时出错: {e}")
        return False

def load_moe_policy(ckpt_path, env, device):
    """加载MoE策略模型"""
    # 加载配置
...
def play():
    ...
    parser.add_argument("--moe_ckpt_path", type=str, required=True, help="MoE模型权重路径")
    parser.add_argument("--base_ckpt_path", type=str, required=True, help="基础专家模型权重路径")
    parser.add_argument("--arm_ckpt_path", type=str, required=True, help="手臂专家模型权重路径")
    ...
    parser.add_argument("--show_weights", action="store_true", help="显示MoE权重分布")
    ...
    # 放宽终止条件，只保留关键的终止条件
    config["env"]["cfg"]["asset"]["terminate_after_contacts_on"] = [
        "base",  # 只保留基座接触地面终止
        # 移除其他过于严格的终止条件（hip, Head, thigh, base_arm_link, link1-5）
    ]
    ...
    # 加载基础专家模型
    print("Loading base expert model...")
    base_config = OmegaConf.create(
        pickle.load(
            open(os.path.join(os.path.dirname(args.base_ckpt_path), "config.pkl"), "rb")
        )
    )
    # 设置ckpt_dir参数
    base_config["runner"]["ckpt_dir"] = os.path.dirname(args.base_ckpt_path)
    
    # 强制使用简单的ActorCritic架构，不包含MoE层
    base_config["runner"]["alg"]["actor_critic"]["_target_"] = "legged_gym.rsl_rl.modules.actor_critic_simple.ActorCritic"
    
    base_runner: OnPolicyRunner = hydra.utils.instantiate(
        base_config["runner"], env=env, eval_fn=None
    )
    base_runner.load(args.base_ckpt_path)
    base_policy = base_runner.alg.actor_critic
    base_policy.eval()
    
    # 加载手臂专家模型
    print("Loading arm expert model...")
    arm_config = OmegaConf.create(
        pickle.load(
            open(os.path.join(os.path.dirname(args.arm_ckpt_path), "config.pkl"), "rb")
        )
    )
    # 设置ckpt_dir参数
    arm_config["runner"]["ckpt_dir"] = os.path.dirname(args.arm_ckpt_path)
    
    # 强制使用简单的ActorCritic架构，不包含MoE层
    arm_config["runner"]["alg"]["actor_critic"]["_target_"] = "legged_gym.rsl_rl.modules.actor_critic_simple.ActorCritic"
    
    arm_runner: OnPolicyRunner = hydra.utils.instantiate(
        arm_config["runner"], env=env, eval_fn=None
    )
    arm_runner.load(args.arm_ckpt_path)
    arm_policy = arm_runner.alg.actor_critic
    arm_policy.eval()
        ...
        with torch.inference_mode():
            while True:
                ...
                if args.show_weights:
                    ...
                    # print(f"Step {count}: Base weight: {base_weight:.3f}, Arm weight: {arm_weight:.3f}, Entropy: {weight_entropy:.3f}")
    ...
    with imageio.get_writer(
        ...
        for step_idx in track(range(args.num_steps), description="Playing with MoE"):
            # 使用MoE策略进行推理
            actions = moe_policy.act_inference(obs)
            ...
            if args.show_weights:
                ...
                if step_idx % 50 == 0:  # 每50步打印一次
                    print(f"Step {step_idx}: Base weight: {base_weight:.3f}, Arm weight: {arm_weight:.3f}, Entropy: {weight_entropy:.3f}")
    ...
    # 保存权重统计信息
    if args.show_weights:
        weight_stats_path = os.path.join(wandb.run.dir, "weight_stats.pkl")
        with open(weight_stats_path, 'wb') as f:
            pickle.dump(weight_stats, f)
        print(f"Weight statistics saved to: {weight_stats_path}")
