...
def play():
    ...
    parser.add_argument("--robot_type", type=str, default="humanoid", choices=["humanoid", "biped"])
    ...
    # 针对人型机器人的特殊配置
    if args.robot_type == "humanoid":
        # 调整初始位置，适合人型机器人
        config["env"]["cfg"]["init_state"]["pos"] = [0.0, 0.0, 1.0]  # 人型机器人通常更高
        
        # 调整关节限制和控制器参数
        if "constraints" in config["env"] and "joint_limit" in config["env"]["constraints"]:
            # 人型机器人的关节限制通常更严格
            config["env"]["constraints"]["joint_limit"]["lower"]["data"] = [
                # 左腿
                -0.5, -0.5, -0.5,  # 髋关节
                -1.0, -1.0, -1.0,  # 膝关节
                -0.5, -0.5, -0.5,  # 踝关节
            ...
            config["env"]["constraints"]["joint_limit"]["upper"]["data"] = [
                # 左腿
                0.5, 0.5, 0.5,  # 髋关节
                1.0, 1.0, 1.0,  # 膝关节
                0.5, 0.5, 0.5,  # 踝关节
    ...
    def update_cam_pos():
        ...
        # 针对人型机器人调整相机位置
        if args.robot_type == "humanoid":
            offset = np.array([1.5, 0.5, 0.8]) * 1.2  # 人型机器人更高，需要调整相机
        else:
